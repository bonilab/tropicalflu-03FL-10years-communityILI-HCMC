...
load("US HHS Regions ILI percent and Zeta score.Rdata")
source("func.R")
...
# N = 4 cycle = 210 and N = 8 cycle = 365 has best fit for our data
# Only try N = 4/8 in US data 
all_cycle = seq(21,64,1)
all_N = c(5,8)
iters = 120
...
for (row_index in 1:nrow(node_args)) {
    ...
    ret[[row_index]] %<-% {
        ...
        for (i in 1:iters) {
            all_fits[[i]] %<-% fit_multi_stepfunc(data = hhs_regions[[10]]$zeta_score,
        ...
        out_file = paste0('best_fit_r10_', row_index, '.RData')
