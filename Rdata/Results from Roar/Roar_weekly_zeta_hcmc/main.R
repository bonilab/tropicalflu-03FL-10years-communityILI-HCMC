...
load("Weekly zeta from averaging daily zeta in a week.Rdata")
source("func.R")
...
# N = 5 cycle = 210 and N = 8 cycle = 365 has best fit for our data
# Only try N = 5/8 
...
for (row_index in 1:nrow(node_args)) {
    ...
    ret[[row_index]] %<-% {
        ...
        for (i in 1:iters) {
            all_fits[[i]] %<-% fit_multi_stepfunc(data = zeta_weekly_avg_df$weekly_zeta,
